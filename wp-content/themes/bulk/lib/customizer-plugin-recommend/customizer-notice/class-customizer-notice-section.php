<?php
/**
 * Bulk Customizer Notification Section Class.
 *
 * @package Bulk
 */

/**
 * bulk_Customizer_Notice_Section class
 */
class bulk_Customizer_Notice_Section extends WP_Customize_Section {
	/**
	...
	public function json() {
		...
		foreach ( $bulk_recommended_plugins as $slug => $plugin_opt ) {
			...
			$bulk_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$bulk_recommended_plugin['class'] = 'active';
			} else {
				$bulk_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$bulk_recommended_plugin['button_class'] = 'install-now button';
					$bulk_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$bulk_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$bulk_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$bulk_recommended_plugin['button_class'] = 'deactivate-now button';
					$bulk_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
			$info   = $this->call_plugin_api( $slug );
			$bulk_recommended_plugin['id'] = $slug;
			$bulk_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$bulk_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$bulk_recommended_plugin['description'] = $info->short_description;
			}

			$bulk_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $bulk_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="bulk-recommeded-actions-container bulk-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="bulk-recommeded-actions">
